...
- **Verify You Can Connect to Kibana**
  ...
  - Log in as the **Elastic admin** using the `elastic` username and the password retrieved from the [Retrieving Passwords section](/docs/install/normal-install#51-retrieving-passwords) of the Install document.
...
### Podman Commands Hang (SQLite Lock Deadlock)
...
Each dashboard and its visualization objects are contained within a NDJSON file (previously JSON) and can be easily imported. The NDJSON files are available in the [dashboards/](https://github.com/cisagov/lme/tree/main/config/dashboards) directory.

![Importing Objects](/img/import.png)

![Importing Objects](/img/import1.png)

![Importing Objects](/img/import2.png)

<div style={{textAlign: "center"}}>Steps to import objects</div>
...
![Check Default Index](/img/stack-management.png)

- Click **Index Patterns** under Kibana Stack Management.

![Check Default Index](/img/index-patterns.png)

- Verify that the **Default label** is set next to the ```INDEX_NAME-*``` index pattern.

![Check Default Index](/img/default-winlogbeat.png)
...
![Set Default Index](/img/default-index-pattern.png)
...
![Check Default Index](/img/fleetservermissingurl.png)
...
For more information, follow the instructions detailed [here](/docs/maintenance/certificates#regenerating-self-signed-certificates).
