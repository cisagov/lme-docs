...
Please reference the [Architecture document](/docs/reference/architecture/) for a detailed description of LME's architecture and its components.
...
- A Linux server - this is where the LME stack (e.g., Elastisearch, Kibana) will be installed via Podman containers
   ...
   **Note: For more information on supported Linux distributions, reference [Supported Linux Distributions](/docs/reference/change-me/).**
...
To ensure reliable performance during installation and operation, your system must meet the following minimum hardware requirements for running LME:
   ...
   - **Storage**
     ...
     - Required for LME data and OS; more is needed if supporting ~17 clients
       ...
       **Note: If your system has less than 16GB of RAM, reference the [Troubleshooting LME Install](/docs/reference/troubleshooting#memory-in-containers-need-more-ramless-ram-usage) for more information on how to configure Podman memory quotas.**
...
Please reference [LME in the Cloud](/docs/logging-guidance/cloud/) for more information on firewalls . 

You must ensure that the client machine(s) you want to monitor can reach the main LME ports as described in the [Required Ports section](/docs/reference/architecture#required-ports) of the Architecture documentation.
