...
While intended for small to medium-sized organizations with limited resources, anyone can download and use LME. Reference [LME 2.0 Prerequisites documentation](/docs/prerequisites) for more details on required infrastructure and hardware including CPU, memory, and storage requirements.
...
LME has an upgrade process from v1.4 -> 2.0. The upgrade uninstalls 1.4 and installs 2.0 and will reintegrate old dashboards and data into the new 2.0 deployment. Check out our [Upgrading documentation](/docs/maintenance/upgrading/) for more information on upgrading from an older version of LME to LME 2.0.

## 8. How do I download LME?

Detailed installation and download steps can be found on the [Install documentation](/docs/install/normal-install).
...
For existing LME users, [click here](/docs/maintenance/upgrading/) for easy instructions on transferring log history from previous versions. LME will automatically reintegrate your log history and data.
...
Although system requirements are mostly the same for LME 2.0, we do have  hardware and infrastructure recommendations in our [LME 2.0 Prerequisite documentation](/docs/prerequisites)
