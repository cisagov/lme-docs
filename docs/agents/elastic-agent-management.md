...
- **Access the Fleet Menu**
   ...
   - Navigate to the **LME dashboard**: `https://{SERVER_IP}`
      - Password information can be found in the [Install documentation](/docs/install/normal-install#51-retrieving-passwords).
...
- **Modify the Command if Necessary** (e.g., if certificates have not been added to the trusted store)
   ...
   - Example:
     ...
     ![example-screenshot](/img/insecure-powershell.png)
...
:::warning
Important: If you have Sysmon installed on your endpoints, ensure **Sysmon Operational** is selected to collect Sysmon logs.
:::
